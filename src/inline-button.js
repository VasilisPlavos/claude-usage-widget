...
export function setInlineButtonActive(active) {
  ...
  if (btn) btn.style.opacity = active ? OPACITY_ACTIVE : OPACITY_REST;
}
...
function injectBtn(headingEl, onToggle) {
  ...
  btn.addEventListener("mouseenter", () => { btn.style.opacity = OPACITY_ACTIVE; });
  btn.addEventListener("mouseleave", () => { btn.style.opacity = OPACITY_REST; });
  btn.addEventListener("click", (e) => { e.stopPropagation(); onToggle(); });
  headingEl.appendChild(btn);
}

function startObserver(onToggle, onInjected) {
  const tryInject = () => {
