...
if (!["major", "minor", "patch"].includes(bumpType)) {
  console.error("Usage: npm run build [patch|minor|major]");
  process.exit(1);
}
...
const zipPath = join(ROOT, "dist", `claude-usage-pip-v${newVersion}.zip`);
...
output.on("close", () =>
  console.log(`dist/claude-usage-pip-v${newVersion}.zip  (${archive.pointer()} bytes)`)
);
archive.finalize();
